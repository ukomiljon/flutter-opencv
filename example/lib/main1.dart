...
class _MyAppState extends State<MyApp> {
  ...
  Future loadImage() async {
    ...
    setState(() { 
      image = Image.file(
          file); //Image(image: assetsImage, fit: BoxFit.cover); // Image.file(file);
      preloaded = true;
    });
  }
    ...
    try {
      switch (functionName) {
        ...
        case 'findContours':
          ...
          try {
            res = await ImgProc.findContours(
                await res, ImgProc.cvRetrTree, ImgProc.cvChainApproxsimple);
          } catch (e) {
            print(e);
          }

          // ImgProc.contoursMatchI1()
          // cnts, hierarchy =   cv2.findContours(edged.copy(),cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
          // res =  await ImgProc.dilate(await res, 40, 60) ;
          // res =  await ImgProc.erode(await res, 40, 60) ;
          // res =  await ImgProc.c .erode(await res, 40, 60) ;
